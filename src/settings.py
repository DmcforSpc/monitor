...
class Settings(BaseSettings):
    ...
    # ── Security middleware (opt-in) ────────────────────────────────
    # Comma-separated lists in env vars, e.g.
    #   CVE_ALLOWED_HOSTS="cve.example.com,localhost"
    #   CVE_CORS_ORIGINS="https://app.example.com"
    # Empty (default) disables the respective middleware.
    # NoDecode tells pydantic-settings to skip its default JSON parsing so
    # our _split_csv validator below can handle the comma-separated form.
    ...
    def _split_csv(cls, v: object) -> object:
        """Allow comma-separated env values like ``a.com,b.com``."""
        if isinstance(v, str):
